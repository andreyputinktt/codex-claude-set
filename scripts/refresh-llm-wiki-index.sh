...
write_managed_block() {
...

  {
    ...
    echo "## Индекс папок"
    echo
    echo "Генерируется из README каждой папки верхнего уровня. Отвечает на «зачем заходить»: выбери папку → читай \`<папка>/README.md\` за деталями. Индекс ведёт только на один уровень вниз — внутреннее устройство папки описывает её собственный README, а не этот блок. Обновить: \`bash codex-claude-set/scripts/refresh-llm-wiki-index.sh --root /Users/a.putinkt-team.de/GIT --index-only\`."
    echo
